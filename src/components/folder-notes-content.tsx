...
import { getNotesByFolder } from "@/server/notes";
import { Button } from "./ui/button";

export function FolderNotesContent() {
  const { id } = useParams();
  ...
  const {
    ...
    queryKey: [id],
    queryFn: ({ queryKey }) => getNotesByFolder(queryKey[0] as string),
  });
  ...
  if (isError) {
    return (
      <Card className="col-span-full p-6">
        ...
        <CardDescription>
          There was a problem loading your notes in this folder. Please try
          again later.
        </CardDescription>
      </Card>
    );
